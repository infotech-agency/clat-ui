...
//   // Fetch active banners from backend (already sorted by order)
//   useEffect(() => {
//     (async () => {
//       try {
//         const res = await fetch(`${API_BASE}/api/hero-banners`, { cache: 'no-store' });
//         console.log("res", res)
...
//   const resolveHref = (slide) => {
//     if (!slide.link) return '#';
//     if (slide.linkType === 'course' && !slide.link.startsWith('/') && !slide.link.startsWith('http')) {
//       return `/courses/${slide.link}`;
//     }
//     return slide.link;
//   };

//   // Nothing to show yet (loading) or no active banners -> render nothing (keeps layout clean)
//   if (loading) {
//     return (
//       <section className="relative h-[70vh] w-full animate-pulse bg-navy md:h-[80vh]" />
//     );
//   }
...
//             {/* Optional overlay text/CTA if admin filled title/subtitle */}
//             {(slide.title || slide.subtitle || slide.buttonText) && (
//               <div className="absolute inset-0 flex items-end bg-gradient-to-t from-navy/80 via-navy/20 to-transparent md:items-center">
//                 <div className="container mx-auto px-4 pb-10 md:pb-0">
//                   <div className="max-w-xl">
//                     {slide.title && (
//                       <h1 className="text-balance font-heading text-3xl font-extrabold leading-tight text-white md:text-5xl">
//                         {slide.title}
//                       </h1>
//                     )}
//                     {slide.subtitle && (
//                       <p className="mt-4 max-w-lg text-sm text-white/80 md:text-lg">
//                         {slide.subtitle}
//                       </p>
//                     )}
//                     {slide.buttonText && (
//                       <span className="btn-orange mt-6 inline-flex">{slide.buttonText}</span>
//                     )}
//                   </div>
//                 </div>
//               </div>
...
export function HeroSlider() {
  ...
  if (loading) {
    return <div className="aspect-[16/9] w-full animate-pulse bg-navy" />;
  }
  ...
  return (
    <section className="relative w-full bg-navy">
      <style>{`
        ...
        .hero-fade { animation: heroFade 0.5s ease-in-out; }
      `}</style>

      {/* Only the active slide is rendered — image keeps its own natural ratio, so it's NEVER cropped */}
      <Link key={slide._id} href={resolveHref(slide)} className="hero-fade block w-full">
        {/* Desktop banner */}
        <img
          src={slide.desktopImage}
          alt={slide.title || 'Hero banner'}
      ...
      {/* Navigation controls */}
      {slides.length > 1 && (
        <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-3 md:bottom-5">
          <button
            onClick={prev}
            aria-label="Previous slide"
            className="flex h-8 w-8 items-center justify-center rounded-full border border-white/30 bg-black/30 text-white transition-colors hover:border-orange hover:bg-orange hover:text-navy md:h-10 md:w-10"
          >
