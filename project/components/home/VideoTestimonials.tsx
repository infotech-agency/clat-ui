...
export function VideoTestimonials() {
  ...
  <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
    {testimonials.map((t) => {
      ...
      return (
        <div key={t._id || t.youtubeUrl} className="overflow-hidden rounded-2xl bg-white/5 border border-white/10">
          <div className="relative aspect-[9/16] bg-navy-800">
            {isPlaying && ytId ? (
              <iframe
                src={`https://www.youtube.com/embed/${ytId}?autoplay=1`}
                title={t.title}
          ...
          <div className="p-4">
            ...
            <h3 className="mt-2 text-sm font-heading font-semibold text-white line-clamp-1">{t.title}</h3>
            <p className="mt-1 text-xs text-white/60 leading-relaxed line-clamp-2">{t.description}</p>
          </div>
        </div>
      );
    })}
